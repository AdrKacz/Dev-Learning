...
# Wrong Answer ... WHY ? Works for test case
# Work when I remove the index jump... why ? KICK and START cannot overlap (no letter in common)
# Penalty: 02:08:53

for t in range(ii()):
    substring = input().upper()
    kick = 'KICK'
    start = 'START'
    l_kick = 4
    l_start = 5
    ...
    while i < len(substring):
        if substring[i: i + l_kick] == kick:
            kick_count += 1
            # i += l_kick
        if substring[i: i + l_start] == start:
            kickstarts += kick_count
        i += 1
        #     i += l_start
        # else:
        #     i += 1
